    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.status === 200 && res.data?.success && res.data?.data) {
                ...
                if (!accessToken) {
                    throw new Error("Response Content Length Mismatch Error");
                }
                ...
                // Decode JWT để lấy userId và role từ claims
                let effectiveRole = "driver";  // Fallback
                let userId = null;

                try {
                    const decoded: any = jwtDecode(accessToken);
                    console.log("Decoded JWT payload:", decoded);  // { sub, userId, role, exp, iat }

                    userId = decoded.userId || decoded.id;  // Numeric từ claim
                    effectiveRole = decoded.role || "DRIVER";  // Enum name từ claim

                    if (userId) {
                        localStorage.setItem("userId", userId.toString());
                    }
                    localStorage.setItem("role", effectiveRole.toLowerCase());  // "driver", "staff", "admin"
                    localStorage.setItem("email", decoded.sub);  // Lưu email từ sub
                } catch (decodeErr: any) {
                    console.error("JWT decode failed:", decodeErr);
                    localStorage.setItem("role", "driver");
                    localStorage.setItem("userId", "0");  // Fallback 0
                }
                ...
                if (roleLower === "driver") {
                    ...
                    console.warn("Unknown role → fallback driver:", effectiveRole);
                    onLogin?.();
                }
            } else {
            ...
            <div className="w-full max-w-md">
                ...
                <div className="bg-card/80 backdrop-blur-sm border border-border/50 rounded-2xl shadow-2xl shadow-primary/5 p-8 space-y-8">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <Input
                                    id="email"
                                    type="text" // Đổi từ "email" thành "text" vì backend accept cả phone
                                    value={email}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Button variant="outline" className="h-12 bg-card/50 border-border/60 hover:bg-accent/50 hover:border-border rounded-xl transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 flex items-center justify-center space-x-2">
